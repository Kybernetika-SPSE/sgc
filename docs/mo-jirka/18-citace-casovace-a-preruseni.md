...
- původní zdroj: [18. Čítače, časovače a přerušení](../archiv/skripta-kyb/kybernetika/chapters/18.%20Čítače,%20časovače%20a%20přerušení.md)
...
![](images/18-async-vs-sync-citac.png)


Elektronický čítač se většinou realizuje pomocí elektronických **logických obvodů**, především pomocí [**klopných obvodů**](15-kombinacni-a-sekvencni-logika.md). V takovém případě mívá vstup, na který se přivádí časový signál označovaný **C** nebo **CLK**, dále vstup nulování hodnoty **RESET**, který čítač okamžitě vynuluje, a případně vstup **SET** pro nastavení předem určené hodnoty. Na výstupech bývá hodnota reprezentována binárně nebo v BCD kódu.

## Užití čítačů v obvodech
**_Asynchronní čítač vpřed_**
![](images/18-asynchronni-citac-vpred.png)

Řetězec [klopných obvodů T](15-kombinacni-a-sekvencni-logika.md). Klopné obvody byly vytvořeny pomocí [obvodu J-K](15-kombinacni-a-sekvencni-logika.md) připojením obou vstupů na logickou $1$. Jednotlivé klopné obvody mění stav výstupu při každé úběžné hraně na svém hodinovém vstupu. Překlápění obvodů se tedy řídí v podstatě dvěma pravidly:
...
![](images/18-binarni-pocitadlo.png)
...
## Obsluha přerušení
Přijde-li do procesoru signalizace přerušení, je v případě, že je obsluha přerušení povolena, nejprve dokončena právě rozpracovaná strojová instrukce. Potom se na zásobník uloží adresa následující strojové instrukce, která by se vykonala, kdyby k přerušení nedošlo. Následně je podle tabulky přerušení vyvolána příslušná obsluha přerušení. Ta musí po svém dokončení vrátit procesor do takového stavu, aby výpočet přerušené úlohy mohl bezpečně pokračovat. Na konci obsluhy přerušení je proto instrukce návratu, která vyzvedne ze zásobníku návratovou adresu a pokračuje se od místa, kde byl program přerušen.
...
**Softwarové přerušení** je speciální strojová instrukce. Tento typ přerušení je na rozdíl od předchozích dvou synchronní, protože je vyvolán zcela záměrně vložením příslušné instrukce přímo do vykonávaného programu.
