...
- `<disk>:` - změní aktuální disk na požadovaný. Příklad: `C:` nebo `H:`.
- `cd <nazev adresare>` - změní aktuální adresář. Akceptuje relativní nebo absolutní cestu. Symbol `..` označuje nadřazený adresář. Příklad: `h:> cd kybernetika`, `h:\kybernetika> cd ..`.
...
- `git fetch` - Aktualizuje informace o vzdáleném repozitáři.
...
- `git switch -` - Vrátí pohled na poslední pozici repozitáře (na aktuální `HEAD`).
- `git reset` - Nastaví `HEAD` repozitáře.
